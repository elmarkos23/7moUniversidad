...
CREATE TABLE empleado (
 ...
 --clave primaria departamento
 alter table dbo.departamento
 add
 constraint pk_departamento primary key nonclustered(numdep)
 --clave primaria y foranea empleado
 alter table dbo.empleado
 add
 constraint pk_empleado primary key nonclustered (nuempl),
 constraint fk_empleado_departamento foreign key(dept) 
 references dbo.departamento(numdep)
...
--departamento
INSERT INTO departamento ( numdep, nomdep, numdirec )
    	VALUES 	( 'A00', 'CENTRO PROCESOS', 000010 ),
    		...
    		( 'E21', 'SOFTWARE', 000100 );
--empleado
 INSERT INTO empleado ( nuempl, nombre, inicial, apellido, dept, tlfn, feching, codtra, niveduc, sexo, fechnac, salario )
 	VALUES ( 10, 'CRISTINA', 'I', 'HERNANDEZ', 'A00', 2070, '01.01.1965', 66, 18, 'M', '14.08.1933', 5275 ),
 		...
 		( 999, 'ANGEL', 'E', 'MALDONADO', 'E11', 3331, '22.03.1988', 69, 20, 'H', '12.11.1965', 7500 );


GO

select * from empleado
select * from departamento where numdep='A00'
select * from log_departamento

INSERT INTO departamento ( numdep, nomdep, numdirec )
    	VALUES 	( 'A99', 'CENTRO Atencion', 000099 )

update departamento
set nomdep='Centro'
where numdep='A00'

delete from departamento where numdep='A99'

IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[tr_departamento_insert]'))
	DROP TRIGGER [tr_departamento_insert]
GO
...
-- con cada instrucción 
	SET NOCOUNT ON;
	INSERT INTO log_departamento(numdep, nomdep, numdirec, usr_fch_cmb,	usr_cmb, usr_hos, usr_tpo)
	SELECT numdep, nomdep, numdirec,GETDATE(), SUSER_NAME(), HOST_NAME(),'I' FROM inserted
END
GO


IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[tr_departamento_delete]'))
	DROP TRIGGER [tr_departamento_delete]
GO
...
-- con cada instrucción 
	SET NOCOUNT ON;
	INSERT INTO log_departamento(numdep, nomdep, numdirec, usr_fch_cmb,	usr_cmb, usr_hos, usr_tpo)
	SELECT numdep, nomdep, numdirec,GETDATE(), SUSER_NAME(), HOST_NAME(),'D' FROM deleted
END
GO

IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[tr_departamento_update]'))
	DROP TRIGGER [tr_departamento_update]
GO
...
-- con cada instrucción 
	SET NOCOUNT ON;
	INSERT INTO log_departamento(numdep, nomdep, numdirec, usr_fch_cmb,	usr_cmb, usr_hos, usr_tpo)
	SELECT numdep, nomdep, numdirec,GETDATE(), SUSER_NAME(), HOST_NAME(),'U' FROM deleted
END
GO

SELECT * FROM dbo.log_departamento